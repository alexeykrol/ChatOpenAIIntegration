...
import { Send, Square, Bot, User, Zap, Copy, Check } from 'lucide-react';
...
export const ChatArea: React.FC = () => {
  ...
  useEffect(() => {
    // Auto-focus input after any dialog closes
    if (!showMemorySettings && !showPersonalities && !showSettings && textareaRef.current) {
      // Small delay to ensure dialog is fully closed
      setTimeout(() => {
        textareaRef.current?.focus();
      }, 100);
    }
  }, [showMemorySettings, showPersonalities, showSettings]);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded-lg transition-colors"
            >
              Personality
            </button>
            ...
            {activePersonality && (
              <button
                ...
                className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded-lg transition-colors"
                title={`Memory: ${activePersonality.has_memory ? 'Enabled' : 'Disabled'}${activePersonality.summarization_enabled ? ' + Summarization' : ''}`}
              >
                <span className="hidden sm:inline">Memory</span>
                <div className={`w-2 h-2 rounded-full ${
                  activePersonality.has_memory 
                    ? (activePersonality.summarization_enabled ? 'bg-blue-500' : 'bg-gray-400')
                    : 'bg-gray-300'
                }`}></div>
              </button>
            )}
